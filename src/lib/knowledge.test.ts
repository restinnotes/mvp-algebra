...
import { getPrerequisiteChain, clearCache, __injectMockKnowledgeGraph } from './knowledge';

describe('getPrerequisiteChain', () => {
    beforeEach(() => {
        clearCache();
        __injectMockKnowledgeGraph(null);
    });
    ...
    test('returns empty chain if node not found', () => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        __injectMockKnowledgeGraph({ version: "1.0", categories: [] } as unknown as any);
