    ...
    return (
        <div className="flex-1 bg-[#111216] border border-white/5 rounded-2xl relative flex flex-col overflow-hidden shadow-2xl">
            ...
            <div className="absolute top-3 right-4 z-10 flex gap-2">
                <button onClick={clearPad} className="p-1.5 bg-white/5 hover:bg-white/10 text-white/50 rounded-lg transition-colors focus-visible:ring-2 focus-visible:ring-indigo-500/50 outline-none" title="清空画板" aria-label="清空画板">
                    <RotateCcw size={14} />
                </button>
            </div>
                        ...
                        <div className="w-full max-w-lg space-y-4">
                            ...
                            <div className="flex items-center justify-center gap-4 pt-4">
                                <button
                                    ...
                                    className={`w-14 h-14 rounded-full flex items-center justify-center transition-all focus-visible:ring-2 focus-visible:ring-indigo-500/50 outline-none ${isRecording && recordingTarget === 'strategy' ? 'bg-rose-500 animate-pulse text-white' : 'bg-white/10 text-white/60 hover:bg-white/20'}`}
                                    aria-label={isRecording && recordingTarget === 'strategy' ? "停止录音思路" : "开始录音思路"}
                        ...
                        <div className="absolute top-4 right-4 z-20 flex flex-col gap-2">
                            <div className="p-1 bg-black/40 backdrop-blur-md rounded-xl border border-white/10 flex flex-col gap-1 shadow-2xl">
                                <button
                                    ...
                                    className={`p-2.5 rounded-lg transition-all focus-visible:ring-2 focus-visible:ring-indigo-500/50 outline-none ${activeTool === 'pen' ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/30' : 'text-white/40 hover:text-white/70 hover:bg-white/5'}`}
                                    title="画笔"
                                    aria-label="画笔工具"
                                ...
                                <button
                                    ...
                                    className={`p-2.5 rounded-lg transition-all focus-visible:ring-2 focus-visible:ring-indigo-500/50 outline-none ${activeTool === 'eraser' ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/30' : 'text-white/40 hover:text-white/70 hover:bg-white/5'}`}
                                    title="橡皮擦"
                                    aria-label="橡皮擦工具"
                                ...
                                <button
                                    ...
                                    className="p-2.5 rounded-lg text-white/40 hover:text-rose-400 hover:bg-rose-500/10 transition-all focus-visible:ring-2 focus-visible:ring-indigo-500/50 outline-none"
                                    title="清空画布"
                                    aria-label="清空画布"
                                >
                                    <Trash2 size={18} />
            ...
            {isStrategyApproved && !isSolved && (
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 w-[85%] max-w-2xl bg-[#1e212b]/95 backdrop-blur border border-indigo-500/30 p-2 rounded-2xl flex shadow-2xl items-center gap-2">
                    ...
                    <button
                        ...
                        className={`p-2 rounded-lg transition-all focus-visible:ring-2 focus-visible:ring-indigo-500/50 outline-none ${isAuxCalculating ? 'bg-indigo-500/20 text-indigo-400 animate-pulse' : 'text-white/30 hover:bg-white/5 hover:text-white/60'}`}
                        title="辅助计算 (AI代算当前内容)"
                        aria-label="辅助计算，AI代算当前内容"
                    >
                        <BrainCircuit size={16} />
                    </button>
                    <button
                        onClick={() => toggleRecording('calculation')}
                        className={`p-2 rounded-lg transition-all focus-visible:ring-2 focus-visible:ring-indigo-500/50 outline-none ${isRecording && recordingTarget === 'calculation' ? 'bg-rose-500/20 text-rose-400 animate-pulse' : 'text-white/30 hover:bg-white/5 hover:text-white/60'}`}
                        title="语音输入推导"
                        aria-label={isRecording && recordingTarget === 'calculation' ? "停止录音推导" : "语音输入推导"}
