...
export default function PracticeUI() {
    ...
    // What: Pre-compute a Map for Knowledge Points instead of O(N) array lookups.
    // Why: To prevent an O(N * M) performance drop during React renders.
    // Impact: Filtering reduces complexity from O(N * M) to O(N + M).
    const kpMap = useMemo(() => {
        const map = new Map<string, KP>();
        allKPs.forEach(kp => map.set(kp.id, kp));
        return map;
    }, [allKPs]);

    // ⚡ Bolt Performance Optimization:
    // What: Cache the filtered question list to prevent recalculation.
    // Why: `Array.filter()` and string processing were previously re-run on every render.
    // Impact: Drastically cuts down render latency during rapid typing in the search bar.
