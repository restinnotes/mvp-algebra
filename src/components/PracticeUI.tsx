...
export default function PracticeUI() {
    ...
    // ⚡ Bolt: Memoize weakKPs to avoid O(N) recalculation on every re-render
    const weakKPs = useMemo(() => {
        return studentData ? Object.keys(studentData.mastery).filter(kp => studentData.mastery[kp] < 0.6) : [];
    }, [studentData]);

    // ⚡ Bolt: Memoize filteredQuestions to avoid expensive string operations and O(N) filtering on every re-render
