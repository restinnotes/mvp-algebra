...
export default function PracticeUI() {
    ...
    // What: Pre-calculate a Hash Map for O(1) Knowledge Point (KP) lookups
    // Why: Replaces O(N) `allKPs.find()` calls inside loops (which caused O(N*M) complexity during rendering and filtering)
    // Impact: Significantly reduces CPU overhead and prevents unnecessary re-renders when rendering large lists of questions
