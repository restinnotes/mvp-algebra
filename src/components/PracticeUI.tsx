...
export default function PracticeUI() {
    ...
    // Pre-calculate an O(1) lookup map for knowledge points instead of using an O(N) array.find()
    // inside nested rendering loops (.filter/.map), which reduces the time complexity
    // from O(N * M) to O(N + M) and minimizes unnecessary re-renders when rendering large question lists.
